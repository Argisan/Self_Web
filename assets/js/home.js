(() => {
  async function fetchSameOriginJson(url, options = {}) {
  ...
  async function checkPortfolioSession() {
    try {
      const { response, data } = await fetchSameOriginJson("/api/auth/session", {
        method: "GET",
        headers: {},
      });
  ...
  logoutBtn?.addEventListener("click", async () => {
    try {
      await fetchSameOriginJson("/api/auth/logout", {
        method: "POST",
        body: JSON.stringify({}),
      });
    } catch {
      // Ignore logout errors and still send the user back to the login page.
    }
  ...
  function loadCookieStatus() {
    try {
      ...
      /* no stored cookies; leave the default state */
    }
  }
  ...
  generateNFTokenBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      try {
        ...
        // Network or CORS error on proxy; fall through to direct call.
      }
      ...
      if (token) {
        setCookieStatus("NFToken generated", "Magic login link ready. Click below to open it.", "success");
        if (cookieStatusCopy) {
